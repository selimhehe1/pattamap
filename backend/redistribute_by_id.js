...
(async () => {
  console.log('🚀 Walking Street redistribution by ID...\n');

  try {
    // Get all Walking Street establishments with their IDs
    ...
    // New positions: 3 per row on rows 1-9
    const newPositions = [
      // Row 1
    ...
    for (let i = 0; i < establishments.length; i++) {
      ...
      console.log(`Moving "${est.name}" from (${est.grid_row}, ${est.grid_col}) → (${newPos.row}, ${newPos.col})`);
