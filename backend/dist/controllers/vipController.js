...
exports.rejectPayment = exports.getVIPTransactions = exports.verifyPayment = exports.cancelVIPSubscription = exports.getMyVIPSubscriptions = exports.purchaseVIP = exports.getPricingOptions = void 0;
const logger_1 = require("../utils/logger");
const supabase_1 = require("../config/supabase");
const vipPricing_1 = require("../config/vipPricing");
...
const getPricingOptions = async (req, res) => {
    try {
        ...
        if (!(0, vipPricing_1.isValidSubscriptionType)(type)) {
        ...
        const pricingOptions = (0, vipPricing_1.getAllPricingOptions)(type);
        ...
        logger_1.logger.error('Error fetching VIP pricing:', error);
...
exports.getPricingOptions = getPricingOptions;
// =====================================================
...
const purchaseVIP = async (req, res) => {
    try {
        ...
        const { subscription_type, entity_id, duration, payment_method, } = req.body;
        // =====================================================
        // 1. VALIDATION
        // =====================================================
        ...
        // Validate subscription type
        if (!(0, vipPricing_1.isValidSubscriptionType)(subscription_type)) {
            return res.status(400).json({
                error: 'Invalid subscription type',
                message: 'Type must be "employee" or "establishment"',
            });
        ...
        const tier = subscription_type;
        // Validate duration
        if (!(0, vipPricing_1.isValidDuration)(duration)) {
            return res.status(400).json({
        ...
        if (!(0, vipPricing_1.isValidPaymentMethod)(payment_method)) {
            return res.status(400).json({
        ...
        if (subscription_type === 'employee') {
            ...
            const { data: userEmployee } = await supabase_1.supabase
                .from('employees')
                .select('id, user_id')
                .eq('id', entity_id)
                .eq('user_id', userId)
            ...
            if (userEmployee) {
                ...
                hasPermission = true;
                logger_1.logger.debug('VIP purchase authorized: Employee buying for self', {
                    userId,
                ...
                const { data: ownership } = await supabase_1.supabase
          ...
          `)
                ...
                if (hasPermission && ownership) {
                    logger_1.logger.debug('VIP purchase authorized: Owner buying for employee', {
                        userId,
                        employeeId: entity_id,
        ...
        // =====================================================
        const tableName = subscription_type === 'employee'
        ...
        // 4. CALCULATE PRICE
        // =====================================================
        const price = (0, vipPricing_1.calculateVIPPrice)(subscription_type, duration);
        if (price === null) {
            return res.status(400).json({
        ...
        const subscriptionStatus = payment_method === 'admin_grant' ? 'active' : 'pending_payment';
        ...
        const { data: subscription, error: subscriptionError } = await supabase_1.supabase
            .from(tableName)
            .insert(subscriptionData)
            .select()
            .single();
        if (subscriptionError || !subscription) {
            logger_1.logger.error('Error creating VIP subscription:', subscriptionError);
            return res.status(500).json({
        ...
        let promptpayData = null;
        if (payment_method === 'promptpay') {
            if (!(0, promptpayService_1.isPromptPayConfigured)()) {
                logger_1.logger.error('PromptPay not configured but payment method is promptpay');
                return res.status(400).json({
            ...
            const qrResult = await (0, promptpayService_1.generatePromptPayQR)(price, subscription.id);
        ...
        if (transactionError || !transaction) {
            logger_1.logger.error('Error creating payment transaction:', transactionError);
            // Rollback: delete the subscription
            await supabase_1.supabase.from(tableName).delete().eq('id', subscription.id);
            return res.status(500).json({
        ...
        await (0, notificationHelper_1.notifyVIPPurchaseConfirmed)(userId, tier, duration, price);
        return res.status(201).json({
            success: true,
            message: payment_method === 'admin_grant'
        ...
        logger_1.logger.error('Error purchasing VIP:', error);
        return res.status(500).json({
 ...
 */
const getMyVIPSubscriptions = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const { data: employeeSubscriptions } = await supabase_1.supabase
      ...
      `)
        ...
        const { data: establishmentSubscriptions } = await supabase_1.supabase
      ...
      `)
        ...
        logger_1.logger.error('Error fetching VIP subscriptions:', error);
        return res.status(500).json({
...
exports.getMyVIPSubscriptions = getMyVIPSubscriptions;
// =====================================================
...
const cancelVIPSubscription = async (req, res) => {
    try {
        ...
        const { data: updatedSubscription, error: updateError } = await supabase_1.supabase
            .from(tableName)
            .update({
            status: 'cancelled',
        ...
        await (0, notificationHelper_1.notifyVIPSubscriptionCancelled)(userId, subscription.tier, 'Cancelled by establishment owner');
        return res.status(200).json({
            success: true,
        ...
        logger_1.logger.error('Error cancelling VIP subscription:', error);
        return res.status(500).json({
...
exports.cancelVIPSubscription = cancelVIPSubscription;
// =====================================================
...
const verifyPayment = async (req, res) => {
    try {
        ...
        const { data: user } = await supabase_1.supabase.from('users').select('role').eq('id', userId).single();
        ...
        const { data: transaction, error: fetchError } = await supabase_1.supabase
            .from('vip_payment_transactions')
            .select('*')
            .eq('id', transactionId)
            .single();
        ...
        const { error: transactionUpdateError } = await supabase_1.supabase
            .from('vip_payment_transactions')
            .update({
        ...
        const tableName = transaction.subscription_type === 'employee'
            ? 'employee_vip_subscriptions'
            : 'establishment_vip_subscriptions';
        const { data: subscription, error: subscriptionUpdateError } = await supabase_1.supabase
            .from(tableName)
            .update({
            status: 'active',
            payment_status: 'completed',
        ...
        if (subscription) {
            await (0, notificationHelper_1.notifyVIPPaymentVerified)(transaction.user_id, subscription.tier, new Date(subscription.expires_at));
        }
        return res.status(200).json({
            success: true,
    ...
    try {
      ...
      `)
        ...
        if (transactionsError) {
            logger_1.logger.error('Error fetching VIP transactions:', transactionsError);
            return res.status(500).json({
        ...
        const transactionsWithSubscriptions = await Promise.all((transactions || []).map(async (transaction) => {
            const tableName = transaction.subscription_type === 'employee'
                ? 'employee_vip_subscriptions'
                : 'establishment_vip_subscriptions';
        ...
        logger_1.logger.error('Error fetching VIP transactions:', error);
        return res.status(500).json({
            error: 'Failed to fetch VIP transactions',
            message: error instanceof Error ? error.message : 'Unknown error',
        });
...
exports.getVIPTransactions = getVIPTransactions;
// =====================================================
...
const rejectPayment = async (req, res) => {
    try {
        ...
        const { error: updateTransactionError } = await supabase_1.supabase
            .from('vip_payment_transactions')
            .update({
        ...
        const tableName = transaction.subscription_type === 'employee'
            ? 'employee_vip_subscriptions'
            : 'establishment_vip_subscriptions';
        const { error: subscriptionUpdateError } = await supabase_1.supabase
            .from(tableName)
            .update({
            status: 'cancelled',
            ...
            .eq('transaction_id', transactionId);
        if (subscriptionUpdateError) {
            logger_1.logger.error('Error cancelling subscription:', subscriptionUpdateError);
            // Continue anyway - transaction is already rejected
        }
        // Get tier from subscription to notify user
        const { data: subscriptionData } = await supabase_1.supabase
            .from(tableName)
            .select('tier')
            .eq('transaction_id', transactionId)
            .single();
        ...
        if (subscriptionData) {
            await (0, notificationHelper_1.notifyVIPPaymentRejected)(transaction.user_id, subscriptionData.tier, admin_notes);
        }
        return res.status(200).json({
            success: true,
