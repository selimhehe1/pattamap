...
exports.deleteIndependentPosition = exports.updateIndependentPosition = exports.createIndependentPosition = exports.getFreelancesForMap = exports.getIndependentPosition = void 0;
 ...
 */
const getIndependentPosition = async (req, res) => {
    try {
        ...
        if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
            logger_1.logger.error('Error fetching independent position:', error);
            return res.status(500).json({ error: 'Failed to fetch independent position' });
        }
        res.json({ data: data || null });
    }
    catch (error) {
        logger_1.logger.error('Error in getIndependentPosition:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.getIndependentPosition = getIndependentPosition;
/**
 * Get all active freelances for the map
 * @deprecated since v10.3 - Use /api/freelances instead
 */
const getFreelancesForMap = async (req, res) => {
    try {
      ...
      `)
        ...
        if (error) {
            logger_1.logger.error('Error fetching freelances for map:', error);
            return res.status(500).json({ error: 'Failed to fetch freelances' });
        }
        res.json({ data: data || [] });
    }
    catch (error) {
        logger_1.logger.error('Error in getFreelancesForMap:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.getFreelancesForMap = getFreelancesForMap;
/**
 ...
 */
const createIndependentPosition = async (req, res) => {
    try {
        const { employee_id, zone, grid_row, grid_col } = req.body;
        ...
        const { data: existingPosition, error: positionCheckError } = await supabase_1.supabase
            .from('independent_positions')
            .select('id')
            .eq('zone', zone)
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .single();
        if (error) {
            logger_1.logger.error('Error creating independent position:', error);
            return res.status(500).json({ error: 'Failed to create independent position' });
        }
        res.status(201).json({ data });
    }
    catch (error) {
        logger_1.logger.error('Error in createIndependentPosition:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.createIndependentPosition = createIndependentPosition;
/**
 * Update an independent position
 * @deprecated since v10.3 - Freelances now use employment_history with is_freelance flag
 */
const updateIndependentPosition = async (req, res) => {
    try {
        const { employeeId } = req.params;
        const { zone, grid_row, grid_col, is_active } = req.body;
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        // Get current position
        const { data: currentPosition, error: fetchError } = await supabase_1.supabase
            .from('independent_positions')
            .select('*')
            .eq('employee_id', employeeId)
            .eq('is_active', true)
        ...
        if ((zone && zone !== currentPosition.zone) ||
            ...
            const { data: existingPosition } = await supabase_1.supabase
                .from('independent_positions')
                .select('id')
                .eq('zone', newZone)
                .eq('grid_row', newRow)
        ...
        if (grid_row !== undefined)
            updateData.grid_row = grid_row;
        if (grid_col !== undefined)
            updateData.grid_col = grid_col;
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .single();
        if (error) {
            logger_1.logger.error('Error updating independent position:', error);
            return res.status(500).json({ error: 'Failed to update independent position' });
        }
        res.json({ data });
    }
    catch (error) {
        logger_1.logger.error('Error in updateIndependentPosition:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.updateIndependentPosition = updateIndependentPosition;
/**
 * Delete (deactivate) an independent position
 * @deprecated since v10.3 - Freelances now use employment_history with is_freelance flag
 */
const deleteIndependentPosition = async (req, res) => {
    try {
        const { employeeId } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const { data, error } = await supabase_1.supabase
            ...
            .single();
        if (error) {
            logger_1.logger.error('Error deleting independent position:', error);
        ...
        logger_1.logger.error('Error in deleteIndependentPosition:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.deleteIndependentPosition = deleteIndependentPosition;
//# sourceMappingURL=independentPositionController.js.map