...
exports.getFreelanceById = exports.getFreelances = void 0;
 ...
 */
const getFreelances = async (req, res) => {
    try {
        const { page = 1, limit = 20, search, nationality, age_min, age_max, has_nightclub, sort_by = 'vip' } = req.query;
      ...
      `, { count: 'exact' })
        ...
        if (search && typeof search === 'string') {
            const escapedSearch = (0, validation_1.escapeLikeWildcards)(search);
        ...
        if (error) {
            logger_1.logger.error('Error fetching freelances:', error);
        ...
        logger_1.logger.error('Get freelances error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.getFreelances = getFreelances;
/**
...
const getFreelanceById = async (req, res) => {
    try {
        ...
        const { data: employmentHistory, error: ehError } = await supabase_1.supabase
      ...
      `)
            ...
            .eq('is_current', true);
        if (ehError) {
            logger_1.logger.error('Error fetching employment history:', ehError);
        ...
        logger_1.logger.error('Get freelance by ID error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.getFreelanceById = getFreelanceById;
