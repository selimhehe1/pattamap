    ...
    describe('submitVerification', () => {
        beforeEach(() => {
            app.post('/api/employees/:id/verify', (req, res, next) => {
                req.user = mockUser;
                next();
            }, verificationController_1.submitVerification);
        });
        it('should submit verification successfully', async () => {
            ...
            supabase_1.supabase.from.mockImplementation((table) => {
                ...
                if (callCount === 3 && table === 'employee_verifications') {
                    // Insert verification
                    return (0, supabaseMockChain_1.createMockChain)({ data: mockVerification, error: null });
                }
        ...
        it('should return 403 if user is not the employee owner', async () => {
            ...
            supabase_1.supabase.from.mockImplementation(() => (0, supabaseMockChain_1.createMockChain)({ data: employeeOwnedByOther, error: null }));
            const response = await (0, supertest_1.default)(app)
                .post('/api/employees/emp-123/verify')
                .send({ selfie_url: 'https://example.com/selfie.jpg' })
                .expect(403);
        ...
        it('should return 400 if already verified', async () => {
            ...
            supabase_1.supabase.from.mockImplementation(() => (0, supabaseMockChain_1.createMockChain)({ data: verifiedEmployee, error: null }));
            const response = await (0, supertest_1.default)(app)
                .post('/api/employees/emp-123/verify')
                .send({ selfie_url: 'https://example.com/selfie.jpg' })
                .expect(400);
        ...
        it('should return 429 if rate limit exceeded', async () => {
            ...
            supabase_1.supabase.from.mockImplementation((table) => {
                ...
                if (callCount === 2) {
                    return (0, supabaseMockChain_1.createMockChain)({ data: recentAttempts, error: null });
                }
                return (0, supabaseMockChain_1.createMockChain)({ data: null, error: null });
            });
            const response = await (0, supertest_1.default)(app)
        ...
        it('should return 500 on verification insert error', async () => {
            ...
            supabase_1.supabase.from.mockImplementation((table) => {
                ...
                if (callCount === 3) {
                    return (0, supabaseMockChain_1.createMockChain)({ data: null, error: { message: 'Insert failed' } });
                }
                return (0, supabaseMockChain_1.createMockChain)({ data: null, error: null });
            });
            const response = await (0, supertest_1.default)(app)
    ...
    describe('getVerificationStatus', () => {
        beforeEach(() => {
            app.get('/api/employees/:id/verification-status', (req, res, next) => {
                req.user = mockUser;
                next();
            }, verificationController_1.getVerificationStatus);
        });
    ...
    describe('getManualReviewQueue', () => {
        beforeEach(() => {
            app.get('/api/admin/verifications/manual-review', (req, res, next) => {
                req.user = mockAdmin;
                next();
            }, verificationController_1.getManualReviewQueue);
        });
        it('should return manual review queue successfully', async () => {
            ...
            supabase_1.supabase.from.mockImplementation(() => (0, supabaseMockChain_1.createMockChain)({ data: mockQueue, error: null }));
            const response = await (0, supertest_1.default)(app)
    ...
    describe('reviewVerification', () => {
        beforeEach(() => {
            app.patch('/api/admin/verifications/:id/review', (req, res, next) => {
                req.user = mockAdmin;
                next();
            }, verificationController_1.reviewVerification);
        });
        it('should approve verification successfully', async () => {
            ...
            supabase_1.supabase.from.mockImplementation((table) => {
                ...
                if (callCount === 4 && table === 'employees') {
                    return (0, supabaseMockChain_1.createMockChain)({ data: { user_id: 'user-123', name: 'Test' }, error: null });
                }
                return (0, supabaseMockChain_1.createMockChain)({ data: null, error: null });
            });
            const response = await (0, supertest_1.default)(app)
        ...
        it('should return 400 if verification not in manual_review status', async () => {
            ...
            supabase_1.supabase.from.mockImplementation(() => (0, supabaseMockChain_1.createMockChain)({ data: approvedVerification, error: null }));
            const response = await (0, supertest_1.default)(app)
                .patch('/api/admin/verifications/verify-123/review')
                .send({ action: 'approve' })
                .expect(400);
        ...
        it('should return 500 on update error', async () => {
            ...
            supabase_1.supabase.from.mockImplementation(() => {
                ...
                return (0, supabaseMockChain_1.createMockChain)({ data: null, error: { message: 'Update failed' } });
            });
            const response = await (0, supertest_1.default)(app)
                .patch('/api/admin/verifications/verify-123/review')
                .send({ action: 'approve' })
    ...
    describe('getRecentVerifications', () => {
        beforeEach(() => {
            app.get('/api/admin/verifications/recent', (req, res, next) => {
                req.user = mockAdmin;
                next();
            }, verificationController_1.getRecentVerifications);
        });
        it('should return recent verifications', async () => {
            ...
            supabase_1.supabase.from.mockImplementation(() => (0, supabaseMockChain_1.createMockChain)({ data: mockVerifications, error: null }));
            const response = await (0, supertest_1.default)(app)
        ...
        it('should filter by status', async () => {
            supabase_1.supabase.from.mockImplementation(() => (0, supabaseMockChain_1.createMockChain)({ data: [mockVerification], error: null }));
            const response = await (0, supertest_1.default)(app)
    ...
    describe('revokeVerification', () => {
        beforeEach(() => {
            app.delete('/api/admin/employees/:id/verification', (req, res, next) => {
                req.user = mockAdmin;
                next();
            }, verificationController_1.revokeVerification);
        });
        it('should revoke verification successfully', async () => {
            ...
            supabase_1.supabase.from.mockImplementation((table) => {
                callCount++;
                if (callCount === 1 && table === 'employees') {
                    return (0, supabaseMockChain_1.createMockChain)({ data: verifiedEmployee, error: null });
                }
        ...
        it('should return 400 if employee not currently verified', async () => {
            supabase_1.supabase.from.mockImplementation(() => (0, supabaseMockChain_1.createMockChain)({ data: mockEmployee, error: null }));
            const response = await (0, supertest_1.default)(app)
                .delete('/api/admin/employees/emp-123/verification')
                .send({ reason: 'Test reason' })
                .expect(400);
...
//# sourceMappingURL=verificationController.test.js.map