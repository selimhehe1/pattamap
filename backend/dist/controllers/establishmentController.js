...
exports.getEstablishmentEmployees = exports.getEstablishmentCategories = exports.updateEstablishmentLogo = exports.updateEstablishmentGridPosition = exports.deleteEstablishment = exports.updateEstablishment = exports.createEstablishment = exports.getEstablishment = exports.getEstablishments = void 0;
...
const getEstablishments = async (req, res) => {
    try {
        ...
        const validatedParams = (0, validation_1.prepareFilterParams)({
        ...
        const limitValidation = (0, validation_1.validateNumericInput)(validatedParams.limit, 1, 200);
        ...
        // Validate page
        const pageValidation = (0, validation_1.validateNumericInput)(validatedParams.page, 1, 10000);
        ...
        if (validatedParams.search) {
            const searchValidation = (0, validation_1.validateTextInput)(validatedParams.search, 0, 100);
      ...
      `);
        ...
        if (establishments && establishments.length > 0) {
            ...
            const { data: employmentData, error: employmentError } = await supabase_1.supabase
        ...
        const establishmentOwnerMap = {};
        if (establishments && establishments.length > 0) {
            const establishmentIds = establishments.map((est) => est.id);
            const { data: ownerData } = await supabase_1.supabase
        ...
        const establishmentsWithCoords = establishments?.map((est) => {
            let latitude = null;
            let longitude = null;
            if (est.location) {
        ...
        function parsePostGISBinary(hexString, establishmentId) {
            try {
                ...
                if (!hexString.startsWith('0101000020E6100000')) {
                    ...
                    logger_1.logger.warn('PostGIS: Unknown WKB format', {
                        establishmentId,
                ...
                if (coordsHex.length < 32) {
                    logger_1.logger.warn('PostGIS: Coordinates hex too short', {
                        establishmentId,
                ...
                if (latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) {
                    logger_1.logger.warn('PostGIS: Coordinates out of valid range', {
                        establishmentId,
                        latitude,
                        longitude,
                        validLatRange: '[-90, 90]',
                ...
                logger_1.logger.error('PostGIS: Exception during binary parsing', {
                    establishmentId,
        ...
        if (countError) {
            logger_1.logger.warn('Count query failed, using establishments length', { error: countError });
        }
        ...
        logger_1.logger.error('Get establishments error:', error);
...
const getEstablishment = async (req, res) => {
    try {
        const { id } = req.params;
        logger_1.logger.debug('🔍 getEstablishment called with ID:', id);
        const { data: establishment, error } = await supabase_1.supabase
            .from('establishments')
            .select(`
        id,
        name,
      ...
      `)
        ...
        if (establishment.location) {
            try {
                ...
                const { data: coords } = await supabase_1.supabase.rpc('get_lat_lng_from_geography', {
                ...
                logger_1.logger.warn('Could not extract location coordinates:', locationError);
            }
        }
        // ========================================
        // BUG #3 FIX - Keep category_id as INTEGER (no transformation)
        ...
        const { data: consumables, error: consumablesError } = await supabase_1.supabase
      ...
      `)
        ...
        if (consumablesError) {
            logger_1.logger.warn('Could not load consumables:', consumablesError);
        }
        // Format consumables for frontend (pricing.consumables format)
        // Note: Supabase returns nested relations as arrays
        const formattedConsumables = (consumables || []).map((ec) => ({
            id: ec.id,
        ...
        const { data: ownerCheck } = await supabase_1.supabase
            .from('establishment_owners')
            .select('id')
            .eq('establishment_id', id)
            .limit(1)
        ...
        logger_1.logger.error('Get establishment error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
...
const createEstablishment = async (req, res) => {
    try {
        logger_1.logger.debug('🔥 CREATE ESTABLISHMENT V3 - FINAL RELOAD FORCE - Body:', req.body);
        const { name, address, zone, category_id, description, phone, website, opening_hours, instagram, twitter, tiktok, pricing, logo_url, latitude, longitude } = req.body;
        logger_1.logger.debug('🏢 CREATE ESTABLISHMENT - Extracted logo_url:', logo_url);
        ...
        if (latitude !== undefined || longitude !== undefined) {
            ...
            if (lat < 12.8 || lat > 13.1 || lng < 100.8 || lng > 101.0) {
                logger_1.logger.warn('Coordinates outside Pattaya region', { lat, lng });
                return res.status(400).json({
      ...
      `)
        ...
        try {
            ...
            await (0, notificationHelper_1.notifyAdminsPendingContent)('establishment', establishment.name, submitterName, establishment.id);
        }
        catch (notifyError) {
        ...
        await (0, redis_1.cacheDel)(redis_1.CACHE_KEYS.CATEGORIES);
        ...
        logger_1.logger.error('Create establishment error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
...
const updateEstablishment = async (req, res) => {
    try {
        logger_1.logger.debug('🔄 UPDATE ESTABLISHMENT called with ID:', req.params.id);
        logger_1.logger.debug('🔄 UPDATE ESTABLISHMENT body:', req.body);
        logger_1.logger.debug('🔄 UPDATE ESTABLISHMENT user:', req.user);
        const { id } = req.params;
        ...
        const isOwner = await (0, auth_1.isEstablishmentOwner)(req.user.id, id);
        if (!isAdmin && !isCreator && !isOwner) {
            logger_1.logger.warn('Unauthorized establishment update attempt', {
                userId: req.user.id,
                establishmentId: id,
                role: req.user.role
            });
        ...
        logger_1.logger.debug('Establishment update authorized', {
            userId: req.user.id,
            establishmentId: id,
        ...
        if (isOwner && !isAdmin && !isCreator) {
            logger_1.logger.debug('🔒 Checking granular permissions for owner');
            ...
            if (ownershipError || !ownership) {
                logger_1.logger.error('Failed to fetch ownership permissions:', ownershipError);
                return res.status(403).json({
            ...
            logger_1.logger.debug('📋 Owner permissions:', permissions);
            ...
            if (modifyingInfo && !permissions.can_edit_info) {
                logger_1.logger.warn('Owner attempted to edit info without permission', {
                    userId: req.user.id,
                    establishmentId: id,
                    attemptedFields
                });
            ...
            if (modifyingPricing && !permissions.can_edit_pricing) {
                logger_1.logger.warn('Owner attempted to edit pricing without permission', {
                    userId: req.user.id,
                    establishmentId: id,
                    attemptedFields
                });
            ...
            if (modifyingPhotos && !permissions.can_edit_photos) {
                logger_1.logger.warn('Owner attempted to edit photos without permission', {
                    userId: req.user.id,
                    establishmentId: id,
                    attemptedFields
                });
            ...
            logger_1.logger.debug('✅ All permission checks passed for owner');
        }
        ...
        if (updates.pricing?.consumables) {
            logger_1.logger.debug('🔧 Updating consumables for establishment:', id);
            logger_1.logger.debug('📦 New consumables:', updates.pricing.consumables);
            ...
            if (updates.pricing.consumables.length > 0) {
                const consumablesToInsert = updates.pricing.consumables.map((c) => ({
                    establishment_id: id,
                    consumable_id: c.consumable_id,
                    price: c.price
                }));
                const { error: consumablesError } = await supabase_1.supabase
                    .from('establishment_consumables')
                    .insert(consumablesToInsert);
                if (consumablesError) {
                    logger_1.logger.error('❌ Failed to update consumables:', consumablesError);
                }
                else {
                    logger_1.logger.debug('✅ Consumables updated successfully');
                }
            }
        }
        ...
        if (updates.pricing) {
            ...
            if (updates.pricing.rooms !== undefined) {
                ...
                logger_1.logger.debug('🏠 Rooms transformation:', {
        ...
        const cleanUpdates = Object.fromEntries(Object.entries(allowedFields).filter(([_, value]) => value !== undefined));
        // DEBUG: Log logo_url specifically
        logger_1.logger.debug('🎨 LOGO UPDATE DEBUG:', {
        ...
        const { data: updatedEstablishment, error } = await supabase_1.supabase
            .from('establishments')
            .update(cleanUpdates)
            .eq('id', id)
            .select(`
      ...
      `)
            .eq('establishment_id', id);
        if (consumablesError) {
            logger_1.logger.warn('Could not load consumables after update:', consumablesError);
        }
        ...
        await (0, redis_1.cacheDel)(redis_1.CACHE_KEYS.ESTABLISHMENT(id));
        await (0, redis_1.cacheInvalidatePattern)('establishments:*');
        await (0, redis_1.cacheDel)('dashboard:stats');
        res.json({
        ...
        logger_1.logger.error('Update establishment error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
...
const deleteEstablishment = async (req, res) => {
    try {
        ...
        const isOwner = await (0, auth_1.isEstablishmentOwner)(req.user.id, id);
        if (!isAdmin && !isCreator && !isOwner) {
            logger_1.logger.warn('Unauthorized establishment delete attempt', {
                userId: req.user.id,
                establishmentId: id,
                role: req.user.role
            });
        ...
        logger_1.logger.debug('Establishment delete authorized', {
            userId: req.user.id,
            establishmentId: id,
            authReason: isAdmin ? 'admin/moderator' : isCreator ? 'creator' : 'owner'
        });
        ...
        logger_1.logger.error('Delete establishment error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.deleteEstablishment = deleteEstablishment;
const updateEstablishmentGridPosition = async (req, res) => {
    try {
        logger_1.logger.debug('🔍 DEBUG - Raw request body type:', typeof req.body);
        logger_1.logger.debug('🔍 DEBUG - Raw request body:', req.body);
        logger_1.logger.debug('🔍 DEBUG - Request headers:', req.headers);
        const { id } = req.params;
        const { grid_row, grid_col, zone, swap_with_id } = req.body;
        logger_1.logger.debug('🎯 GRID POSITION UPDATE REQUEST:', {
            establishmentId: id,
        ...
        if (!grid_row || !grid_col || !zone) {
            logger_1.logger.error('❌ VALIDATION FAILED - missing required fields:', { grid_row, grid_col, zone });
            return res.status(400).json({
        ...
        if (!['admin', 'moderator'].includes(req.user.role)) {
            logger_1.logger.error('❌ PERMISSION DENIED:', {
        ...
        const { data: currentEst } = await supabase_1.supabase
            .from('establishments')
        ...
        const { data: existingEst } = await supabase_1.supabase
            .from('establishments')
        ...
        if (swap_with_id && existingEst) {
            ...
            const { error: swapError } = await supabase_1.supabase.rpc('swap_establishment_positions', {
                est1_id: id,
            ...
            logger_1.logger.debug(`🔄 GRID SWAP: ${currentEst.name} (${currentEst.grid_row},${currentEst.grid_col}) ↔ ${existingEst.name} (${grid_row},${grid_col})`);
            return res.json({
        ...
        logger_1.logger.debug('🗄️ UPDATING DATABASE:', {
            establishmentId: id,
            updateData: { grid_row, grid_col, zone },
            operation: 'simple_move'
        });
        const { data: updatedEst, error } = await supabase_1.supabase
            .from('establishments')
            ...
            .single();
        if (error) {
            logger_1.logger.error('❌ DATABASE UPDATE FAILED:', {
                error: error.message,
                establishmentId: id,
                updateData: { grid_row, grid_col, zone }
            });
        ...
        logger_1.logger.debug('✅ DATABASE UPDATE SUCCESS:', {
        ...
        logger_1.logger.debug(`📍 GRID MOVE: ${updatedEst.name} → (${grid_row},${grid_col}) in ${zone}`);
        res.json({
        ...
        logger_1.logger.error('Update grid position error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.updateEstablishmentGridPosition = updateEstablishmentGridPosition;
const updateEstablishmentLogo = async (req, res) => {
    try {
        logger_1.logger.debug('🔧 UPDATE LOGO - Start function');
        logger_1.logger.debug('🔧 UPDATE LOGO - Request params:', req.params);
        logger_1.logger.debug('🔧 UPDATE LOGO - Request body:', req.body);
        logger_1.logger.debug('🔧 UPDATE LOGO - User:', req.user);
        const { id } = req.params;
        const { logo_url } = req.body;
        logger_1.logger.debug('🔧 UPDATE LOGO - Extracted ID:', id);
        logger_1.logger.debug('🔧 UPDATE LOGO - Extracted logo_url:', logo_url);
        // Verify user is admin or moderator
        if (!req.user || !['admin', 'moderator'].includes(req.user.role)) {
            logger_1.logger.debug('❌ UPDATE LOGO - Permission denied for user:', req.user);
            return res.status(403).json({ error: 'Admin or moderator access required' });
        }
        logger_1.logger.debug('✅ UPDATE LOGO - Permission granted for:', req.user.role);
        // Validate establishment ID
        if (!id) {
            logger_1.logger.debug('❌ UPDATE LOGO - Missing establishment ID');
            return res.status(400).json({ error: 'Establishment ID is required' });
        }
        // Validate logo URL
        if (!logo_url || typeof logo_url !== 'string') {
            logger_1.logger.debug('❌ UPDATE LOGO - Invalid logo URL:', { logo_url, type: typeof logo_url });
            return res.status(400).json({ error: 'Valid logo URL is required' });
        }
        logger_1.logger.debug('🗄️ UPDATE LOGO - Updating database...');
        ...
        const { data: establishment, error } = await supabase_1.supabase
            ...
            .single();
        logger_1.logger.debug('🗄️ UPDATE LOGO - Database response:', { data: establishment, error });
        if (error) {
            logger_1.logger.error('❌ UPDATE LOGO - Database update error:', error);
            return res.status(400).json({ error: error.message });
        }
        if (!establishment) {
            logger_1.logger.debug('❌ UPDATE LOGO - Establishment not found');
            return res.status(404).json({ error: 'Establishment not found' });
        }
        logger_1.logger.debug('✅ UPDATE LOGO - Success! Updated establishment:', establishment);
        res.json({
        ...
        logger_1.logger.error('❌ UPDATE LOGO - Catch error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.updateEstablishmentLogo = updateEstablishmentLogo;
const getEstablishmentCategories = async (req, res) => {
    try {
...
exports.getEstablishmentCategories = getEstablishmentCategories;
/**
...
const getEstablishmentEmployees = async (req, res) => {
    try {
        ...
        const { data: ownership, error: ownershipError } = await supabase_1.supabase
            ...
            .single();
        if (ownershipError || !ownership) {
            logger_1.logger.warn('Unauthorized employee list access attempt', {
                userId: user_id,
                establishmentId: id
            });
            return res.status(403).json({
        ...
        logger_1.logger.debug('Establishment employee list access authorized', {
            userId: user_id,
            establishmentId: id,
      ...
      `)
            ...
            .single();
        if (estError) {
            logger_1.logger.error('Establishment not found:', estError);
            return res.status(404).json({ error: 'Establishment not found' });
        }
        const isNightclub = establishment.category?.name === 'Nightclub';
        // 3. Fetch employees via current_employment
        const { data: employments, error: empError } = await supabase_1.supabase
      ...
      `)
            .eq('establishment_id', id);
        if (empError) {
            logger_1.logger.error('Failed to fetch employees:', empError);
        ...
        let employees = employments
            ...
            employee_type: emp.employee.is_freelance ? 'freelance' : 'regular'
        }));
        // 5. v10.3: If nightclub, also fetch associated freelances
        if (isNightclub) {
            logger_1.logger.debug('Nightclub detected, fetching associated freelances', { establishmentId: id });
            const { data: freelanceEmployments, error: freelanceError } = await supabase_1.supabase
                .from('employment_history')
                .select(`
          employee_id,
          start_date,
        ...
        `)
            ...
            if (!freelanceError && freelanceEmployments) {
                const freelances = freelanceEmployments
                    .filter(emp => emp.employee && emp.employee.is_freelance === true)
                    .map(emp => ({
                    ...emp.employee,
                    current_employment: {
                        establishment_id: id,
                ...
                const newFreelances = freelances.filter((f) => !existingIds.has(f.id));
                employees = [...employees, ...newFreelances];
                logger_1.logger.debug('Freelances merged', {
        ...
        try {
            ...
            const { data: vipSubs } = await supabase_1.supabase
            ...
            const vipMap = new Map((vipSubs || []).map((sub) => [sub.employee_id, sub.expires_at]));
        ...
        logger_1.logger.debug('Successfully fetched employees', {
            establishmentId: id,
        ...
        logger_1.logger.error('Get establishment employees error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
exports.getEstablishmentEmployees = getEstablishmentEmployees;
