...
exports.validateEstablishmentIsNightclub = validateEstablishmentIsNightclub;
exports.validateAllEstablishmentsAreNightclubs = validateAllEstablishmentsAreNightclubs;
exports.validateFreelanceRules = validateFreelanceRules;
exports.getFreelanceNightclubs = getFreelanceNightclubs;
 ...
 */
async function validateEstablishmentIsNightclub(establishmentId) {
    try {
      ...
      `)
        ...
        const categoryName = establishment.category?.name || null;
        ...
        logger_1.logger.error('Error validating establishment category:', err);
        return {
            isNightclub: false,
            categoryName: null,
 ...
 */
async function validateAllEstablishmentsAreNightclubs(establishmentIds) {
    try {
        if (establishmentIds.length === 0) {
            return { valid: true, invalidEstablishments: [], error: null };
        }
        const { data: establishments, error } = await supabase_1.supabase
            .from('establishments')
            .select(`
        id,
        name,
      ...
      `)
        ...
        logger_1.logger.error('Error validating establishments:', err);
        return {
            valid: false,
            invalidEstablishments: [],
 ...
 */
async function validateFreelanceRules(employeeId, isFreelance, establishmentIds = []) {
 ...
 */
async function getFreelanceNightclubs(employeeId) {
    try {
        const { data: employmentHistory, error } = await supabase_1.supabase
      ...
      `)
        ...
        if (error || !employmentHistory) {
            logger_1.logger.error('Error fetching freelance nightclubs:', error);
            return [];
        }
        // Filter only nightclubs
        const nightclubIds = employmentHistory
            .filter((eh) => eh.establishments?.category?.name === 'Nightclub')
            .map((eh) => eh.establishment_id);
        return nightclubIds;
    }
    catch (err) {
        logger_1.logger.error('Error in getFreelanceNightclubs:', err);
        return [];
    }
}
//# sourceMappingURL=freelanceValidation.js.map