...
const createDefaultChain = supabaseMockChain_1.createMockChain;
// Helper to mock auth + additional calls
const mockSupabaseAuthLocal = (user, additionalMocks) => {
    let callCount = 0;
...
describe('🧪 Admin Routes - Comprehensive Test Suite', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        adminToken = jsonwebtoken_1.default.sign({ userId: mockAdminUser.id, email: mockAdminUser.email, role: 'admin' }, process.env.JWT_SECRET, { expiresIn: '1h' });
        moderatorToken = jsonwebtoken_1.default.sign({ userId: mockModeratorUser.id, email: mockModeratorUser.email, role: 'moderator' }, process.env.JWT_SECRET, { expiresIn: '1h' });
        userToken = jsonwebtoken_1.default.sign({ userId: mockRegularUser.id, email: mockRegularUser.email, role: 'user' }, process.env.JWT_SECRET, { expiresIn: '1h' });
    });
    // ==========================================
        ...
        describe('DELETE /establishments/:id', () => {
            ...
            it('should handle cascade deletion', async () => {
                ...
                await (0, supertest_1.default)(app)
                    .delete('/api/admin/establishments/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa')
                    .set('Cookie', [`auth-token=${adminToken}`]);
                expect(supabase_1.supabase.from).toHaveBeenCalledWith('establishments');
            });
    ...
    describe('💬 Comments', () => {
        ...
        describe('POST /comments/:id/approve', () => {
            ...
            it('should clear reports on approval', async () => {
                ...
                expect(supabase_1.supabase.from).toHaveBeenCalledWith('comments');
            });
        });
    ...
    describe('🍹 Consumables', () => {
        ...
        describe('GET /consumables', () => {
            ...
            it('should order by category and name', async () => {
                ...
                expect(supabase_1.supabase.from).toHaveBeenCalledWith('consumable_templates');
            });
            it('should handle DB errors', async () => {
                mockSupabaseAuth(mockAdminUser, (table, callCount) => {
                    if (table === 'consumable_templates' && callCount > 1) {
        ...
        describe('DELETE /establishments/:establishment_id/consumables/:consumable_id', () => {
            ...
            it('should not delete consumable template', async () => {
                ...
                await (0, supertest_1.default)(app)
                    ...
                    .set('Cookie', [`auth-token=${adminToken}`])
                    .expect(200);
                expect(supabase_1.supabase.from).toHaveBeenCalledWith('establishment_consumables');
            });
            it('should handle DB errors', async () => {
                mockSupabaseAuth(mockAdminUser, (table, callCount) => {
                    if (table === 'establishment_consumables' && callCount > 1) {
...
//# sourceMappingURL=admin.complete.test.js.map