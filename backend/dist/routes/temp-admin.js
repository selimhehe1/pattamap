...
router.get('/dashboard-stats', async (req, res) => {
    ...
    try {
        ...
        const { count: totalEstablishments } = await supabase_1.supabase
            .from('establishments')
            .select('*', { count: 'exact', head: true });
        const { count: pendingEstablishments } = await supabase_1.supabase
            .from('establishments')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'pending');
        // Compter les employées
        const { count: totalEmployees } = await supabase_1.supabase
            .from('employees')
            .select('*', { count: 'exact', head: true });
        const { count: pendingEmployees } = await supabase_1.supabase
            .from('employees')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'pending');
        ...
        const { count: totalComments } = await supabase_1.supabase
            .from('comments')
            .select('*', { count: 'exact', head: true });
        const { count: pendingComments } = await supabase_1.supabase
            .from('comments')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'pending');
        // Compter les signalements
        const { count: reportedComments } = await supabase_1.supabase
            .from('reports')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'pending');
        const stats = {
        ...
        logger_1.logger.debug('📊 Dashboard stats:', stats);
        res.json(stats);
    }
    catch (error) {
...
router.get('/establishments', async (req, res) => {
    logger_1.logger.debug('🏢 TEMP ADMIN ESTABLISHMENTS ROUTE HIT!');
    try {
        const { status } = req.query;
        logger_1.logger.debug('Filter status:', status);
      ...
      `)
        ...
        logger_1.logger.debug(`Found ${data?.length || 0} establishments`);
...
router.get('/employees', async (req, res) => {
    logger_1.logger.debug('👩 TEMP ADMIN EMPLOYEES ROUTE HIT!');
    try {
        const { status } = req.query;
        let query = supabase_1.supabase
            .from('employees')
...
router.post('/make-admin/:userId', async (req, res) => {
    try {
        ...
        logger_1.logger.debug('🔧 MAKING USER ADMIN:', userId);
        const { data, error } = await supabase_1.supabase
            ...
            .single();
        if (error) {
            logger_1.logger.error('Error making user admin:', error);
            // SECURITY FIX: Don't expose error details
            return res.status(400).json({ error: 'Failed to upgrade user to admin' });
        }
        logger_1.logger.debug('✅ User upgraded to admin:', data);
        res.json({ message: 'User upgraded to admin successfully', user: data });
    }
    catch (error) {
        logger_1.logger.error('Make admin error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
exports.default = router;
//# sourceMappingURL=temp-admin.js.map