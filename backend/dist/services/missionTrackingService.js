...
exports.missionTrackingService = void 0;
...
class MissionTrackingService {
     ...
     */
    async onCheckIn(userId, establishmentId, zone, verified) {
        try {
            if (!verified) {
                // Only count verified check-ins for missions
                logger_1.logger.debug('Check-in not verified, skipping mission tracking');
                return;
            }
            logger_1.logger.debug('Mission tracking: check-in event', { userId, establishmentId, zone });
            // Get active check-in related missions
            const { data: missions, error } = await supabase_1.supabase
                .from('missions')
                .select('*')
                .eq('is_active', true)
                .or('requirements->>type.eq.check_in,requirements->>type.eq.check_in_zone,requirements->>type.eq.check_in_all_zones');
            if (error) {
                logger_1.logger.error('Failed to fetch check-in missions:', error);
                return;
            }
            if (!missions || missions.length === 0) {
                logger_1.logger.debug('No active check-in missions found');
                return;
            }
            // Process each relevant mission in parallel (FIXED: was sequential N+1 queries)
            await Promise.all(missions.map(mission => this.processCheckInMission(userId, mission, establishmentId, zone)));
        }
        catch (error) {
            logger_1.logger.error('Error in onCheckIn mission tracking:', error);
        }
    }
    /**
     ...
     */
    async onReviewCreated(userId, reviewId, reviewLength, hasPhotos) {
        try {
            logger_1.logger.debug('Mission tracking: review created event', { userId, reviewId, reviewLength, hasPhotos });
            ...
            if (error) {
                logger_1.logger.error('Failed to fetch review missions:', error);
                return;
            }
            if (!missions || missions.length === 0) {
                logger_1.logger.debug('No active review missions found');
                return;
            }
            // Process each relevant mission in parallel (FIXED: was sequential N+1 queries)
            await Promise.all(missions.map(mission => this.processReviewMission(userId, mission, reviewLength, hasPhotos)));
        }
        catch (error) {
            logger_1.logger.error('Error in onReviewCreated mission tracking:', error);
        }
    }
    /**
     ...
     */
    async onVoteCast(userId, reviewId, voteType) {
        try {
            if (voteType !== 'helpful') {
                // Only count helpful votes for missions
                return;
            }
            logger_1.logger.debug('Mission tracking: vote cast event', { userId, reviewId, voteType });
            ...
            if (error) {
                logger_1.logger.error('Failed to fetch vote missions:', error);
                return;
            }
            if (!missions || missions.length === 0) {
                logger_1.logger.debug('No active vote missions found');
                return;
            }
            // Process each relevant mission in parallel (FIXED: was sequential updates)
            await Promise.all(missions.map(mission => this.updateMissionProgress(userId, mission.id, 1)));
        }
        catch (error) {
            logger_1.logger.error('Error in onVoteCast mission tracking:', error);
        }
    }
    /**
     ...
     */
    async onFollowAction(followerId, followingId) {
        try {
            logger_1.logger.debug('Mission tracking: follow action event', { followerId, followingId });
            ...
            if (error) {
                logger_1.logger.error('Failed to fetch follow missions:', error);
                return;
            }
            if (!missions || missions.length === 0) {
                logger_1.logger.debug('No active follow missions found');
                return;
            }
            ...
            logger_1.logger.error('Error in onFollowAction mission tracking:', error);
        }
    }
    /**
     ...
     */
    async onHelpfulVoteReceived(reviewAuthorId, reviewId) {
        try {
            logger_1.logger.debug('Mission tracking: helpful vote received event', { reviewAuthorId, reviewId });
            ...
            if (error) {
                logger_1.logger.error('Failed to fetch helpful vote received missions:', error);
                return;
            }
            if (!missions || missions.length === 0) {
                logger_1.logger.debug('No active receive helpful vote missions found');
                return;
            }
            ...
            logger_1.logger.error('Error in onHelpfulVoteReceived mission tracking:', error);
        }
    }
    /**
     ...
     */
    async onPhotoUploaded(userId, photoUrl, entityType, entityId) {
        try {
            logger_1.logger.debug('Mission tracking: photo uploaded event', { userId, photoUrl, entityType, entityId });
            ...
            if (error) {
                logger_1.logger.error('Failed to fetch photo upload missions:', error);
                return;
            }
            if (!missions || missions.length === 0) {
                logger_1.logger.debug('No active photo upload missions found');
                return;
            }
            // Process each relevant mission in parallel (FIXED: was sequential updates)
            await Promise.all(missions.map(mission => this.updateMissionProgress(userId, mission.id, 1)));
            ...
            logger_1.logger.error('Error in onPhotoUploaded mission tracking:', error);
        }
    }
    // ========================================
     ...
     */
    async processCheckInMission(userId, mission, establishmentId, zone) {
        try {
            ...
            logger_1.logger.error('Error processing check-in mission:', error);
        }
    }
    /**
     * Process review related missions
     */
    async processReviewMission(userId, mission, reviewLength, hasPhotos) {
        try {
            const req = mission.requirements;
            ...
            logger_1.logger.error('Error processing review mission:', error);
        }
    }
    // ========================================
    // PROGRESS TRACKING
     ...
     */
    async updateMissionProgress(userId, missionId, increment) {
        try {
            // Call PostgreSQL RPC function for atomic update
            const { data, error } = await supabase_1.supabase.rpc('update_mission_progress', {
                p_user_id: userId,
                p_mission_id: missionId,
                p_increment: increment
            });
            if (error) {
                logger_1.logger.error('Failed to update mission progress:', error);
                return;
            }
            logger_1.logger.debug('Mission progress updated', { userId, missionId, increment, completed: data });
            ...
            logger_1.logger.error('Error updating mission progress:', error);
        }
    }
    /**
     ...
     */
    async setMissionProgress(userId, missionId, progress) {
        try {
            ...
            const { data: completed, error } = await supabase_1.supabase.rpc('set_mission_progress_absolute', {
                p_user_id: userId,
                p_mission_id: missionId,
                p_new_progress: progress
            });
            if (error) {
                logger_1.logger.error('Failed to set mission progress:', error);
                return;
            }
            logger_1.logger.debug('Mission progress set', { userId, missionId, progress, completed });
            ...
            logger_1.logger.error('Error setting mission progress:', error);
        }
    }
    /**
     ...
     */
    async handleMissionCompletion(userId, missionId) {
        try {
            logger_1.logger.info('Mission completed!', { userId, missionId });
            // Get mission details
            const { data: mission, error } = await supabase_1.supabase
                .from('missions')
                .select('name, requirements')
                .eq('id', missionId)
                .single();
            if (error || !mission) {
                logger_1.logger.error('Failed to get completed mission:', error);
                return;
            }
            ...
            logger_1.logger.error('Error handling mission completion:', error);
        }
    }
    /**
     ...
     */
    async unlockNextQuestStep(userId, questId, currentStep) {
        try {
            // Find next step mission
            const { data: nextMission, error } = await supabase_1.supabase
                .from('missions')
                .select('id, name')
            ...
            if (error || !nextMission) {
                logger_1.logger.debug('No next quest step found', { questId, currentStep });
                return;
            }
            // Initialize progress for next step
            const { error: initError } = await supabase_1.supabase
            ...
            if (initError) {
                logger_1.logger.error('Failed to unlock next quest step:', initError);
            }
            else {
                logger_1.logger.info('Next quest step unlocked', { userId, questId, step: currentStep + 1, missionName: nextMission.name });
            }
        }
        catch (error) {
            logger_1.logger.error('Error unlocking next quest step:', error);
        }
    }
    // ========================================
     ...
     */
    async getUniqueCheckInCount(userId, missionType) {
        try {
            ...
            if (error) {
                logger_1.logger.error('Failed to count unique check-ins:', error);
                return 0;
            }
            // Count unique establishments
            const uniqueEstablishments = new Set(data?.map((ci) => ci.establishment_id) || []);
            return uniqueEstablishments.size;
        }
        catch (error) {
            logger_1.logger.error('Error counting unique check-ins:', error);
            return 0;
        }
    }
    /**
     ...
     */
    async getZoneCheckInCount(userId, zone, missionType) {
        try {
            let query = supabase_1.supabase
                .from('check_ins')
            ...
            if (error) {
                logger_1.logger.error('Failed to count zone check-ins:', error);
                return 0;
            }
            return count || 0;
        }
        catch (error) {
            logger_1.logger.error('Error counting zone check-ins:', error);
            return 0;
        }
    }
    /**
     ...
     */
    async getUniqueZonesVisited(userId, missionType) {
        try {
            let query = supabase_1.supabase
                .from('check_ins')
            ...
            const { data, error } = await query;
            if (error) {
                logger_1.logger.error('Failed to count unique zones:', error);
                return 0;
            }
            // Count unique zones
            const uniqueZones = new Set(data?.map((ci) => ci.establishment?.zone).filter(Boolean) || []);
     ...
     */
    async getReviewCount(userId, missionType) {
        try {
            let query = supabase_1.supabase
                .from('comments')
            ...
            const { count, error } = await query;
            if (error) {
                logger_1.logger.error('Failed to count reviews:', error);
                return 0;
            }
            return count || 0;
        }
     ...
     */
    async getQualityReviewCount(userId, minLength, requirePhoto, missionType) {
        try {
            let query = supabase_1.supabase
                .from('comments')
            ...
            const { data, error } = await query;
            if (error) {
                logger_1.logger.error('Failed to count quality reviews:', error);
                return 0;
            }
            // Phase 3: Get all review IDs that have photos (single query, efficient)
            let reviewPhotosSet = new Set();
            if (requirePhoto) {
                const { data: photosData } = await supabase_1.supabase
            ...
            logger_1.logger.error('Error counting quality reviews:', error);
            return 0;
        }
    }
    /**
     ...
     */
    async getReviewsWithPhotosCount(userId, missionType) {
        try {
            let query = supabase_1.supabase
                .from('user_photo_uploads')
                .select('id', { count: 'exact' })
            ...
            const { count, error } = await query;
            if (error) {
                logger_1.logger.error('Failed to count reviews with photos:', error);
                return 0;
            }
            logger_1.logger.debug('Reviews with photos count:', { userId, count, missionType });
            return count || 0;
        }
        catch (error) {
            logger_1.logger.error('Error counting reviews with photos:', error);
            return 0;
        }
    }
    /**
     ...
     */
    async getReviewsMinLengthCount(userId, minLength, missionType) {
        try {
            let query = supabase_1.supabase
                .from('comments')
                .select('id, comment', { count: 'exact' })
            ...
            const { data, error } = await query;
            if (error) {
                logger_1.logger.error('Failed to count min length reviews:', error);
                return 0;
            }
            // Filter by length
            ...
            logger_1.logger.error('Error counting min length reviews:', error);
            return 0;
        }
    }
    // ========================================
    ...
    async resetDailyMissions() {
        try {
            logger_1.logger.info('Resetting daily missions...');
            // Call PostgreSQL RPC function
            const { error } = await supabase_1.supabase.rpc('reset_missions', {
                p_mission_type: 'daily'
            });
            if (error) {
                logger_1.logger.error('Failed to reset daily missions:', error);
            }
            else {
                logger_1.logger.info('Daily missions reset successfully');
            }
        }
        catch (error) {
            logger_1.logger.error('Error resetting daily missions:', error);
        }
    }
    /**
    ...
    async resetWeeklyMissions() {
        try {
            logger_1.logger.info('Resetting weekly missions...');
            // Call PostgreSQL RPC function
            const { error } = await supabase_1.supabase.rpc('reset_missions', {
                p_mission_type: 'weekly'
            });
            if (error) {
                logger_1.logger.error('Failed to reset weekly missions:', error);
            }
            else {
                logger_1.logger.info('Weekly missions reset successfully');
            }
        }
        catch (error) {
            logger_1.logger.error('Error resetting weekly missions:', error);
        }
    }
    // ========================================
    // UTILITY FUNCTIONS
...
// ========================================
exports.missionTrackingService = new MissionTrackingService();
//# sourceMappingURL=missionTrackingService.js.map