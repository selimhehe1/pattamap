...
async function fixRemaining() {
  console.log('🔧 Fixing remaining 2 duplicate positions...\n');

  // Move Toy Box from (1,1) where it conflicts with Konoha to (1,6)
  console.log('📍 Moving "Toy Box" from (1,1) to (1,6)...');
  ...
  if (toyBoxError) {
    ...
    console.log(`   ✅ Successfully moved to (${toyBox[0].grid_row}, ${toyBox[0].grid_col})`);
  }

  // Move Roxy Bar from (2,2) where it conflicts with Horny Bar to (2,3)
  console.log('📍 Moving "Roxy Bar" from (2,2) to (2,3)...');
  ...
  if (roxyBarError) {
    ...
    console.log(`   ✅ Successfully moved to (${roxyBar[0].grid_row}, ${roxyBar[0].grid_col})`);
  }
...
fixRemaining().then(() => process.exit(0));